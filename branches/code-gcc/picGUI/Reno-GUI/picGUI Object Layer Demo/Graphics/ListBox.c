...

{
    ...
    if(pText != NULL){
        ...
        counter = 0;
        while(*pointer){
            if(NULL == LbAddItem(pLb, NULL, pointer, 0, counter))
                break;
            while(*pointer++ > 31);
            if(*(pointer-1) == 0)
                break;
...
/*********************************************************************
* Function: LISTITEM* LbAddItem(LISTBOX *pLb, LISTITEM *pPrevItem, XCHAR *pText, WORD status, WORD data)
*
...
*
********************************************************************/
LISTITEM* LbAddItem(LISTBOX *pLb, LISTITEM *pPrevItem, XCHAR *pText, WORD status, WORD data){
LISTITEM* pItem;
LISTITEM* pCurItem; 
...
if((GetY()+ pLb->textHeight) >= (pLb->top+GOL_EMBOSS_SIZE+LB_INDENT)){
        ...
        case LB_STATE_ERASEITEM:
            ...
            if (!GetState(pLb, EB_CENTER_ALIGN|EB_RIGHT_ALIGN)) {
                MoveTo(pLb->left+GOL_EMBOSS_SIZE+EB_INDENT, GetY());
            }else{
                temp = GetTextWidth(pCurItem->pText, pLb->pGolScheme->pFont);
                if (GetState(pLb, EB_RIGHT_ALIGN)) {
			        MoveTo(pLb->right-temp-EB_INDENT-GOL_EMBOSS_SIZE, GetY());
                }else{
