 ...
 * Processor:       PIC24
 * Compiler:       	MPLAB C30 Version 3.00
 * Linker:          MPLAB LINK30
				  ...
				  XCHAR *pText, GOL_SCHEME *pScheme)				 
{
	...
	if (pText != NULL) {
		...
		tempWidth = GOL_EMBOSS_SIZE+(GetTextWidth("8",pScheme->pFont)*SCALECHARCOUNT);
	}
	else {
		tempHeight = GOL_EMBOSS_SIZE;
		tempWidth = GOL_EMBOSS_SIZE;	
	}
		
	// Meter size is dependent on the width or heigh.
	// The radius is also adjusted to add space for the scales 

#if defined (MTR_WHOLE_TYPE) 
	// choose the radius 
	if ((right-left) > (bottom-top))
		pMtr->radius = ((bottom-top-tempHeight)>>1) - 									\
						(GetTextHeight(pScheme->pFont)+((bottom-top-tempHeight)>>3));
	else	
	...
	if (((right-left)>>1) > (bottom-top))
		pMtr->radius = (bottom-top) - (GOL_EMBOSS_SIZE+tempHeight+((bottom-top)>>2));
	else	
		pMtr->radius = ((right-left)>>1) - (tempWidth+((right-left)>>3));

	// center the meter on the given dimensions
	pMtr->xCenter = (left+right)>>1;
	pMtr->yCenter = bottom - (((bottom - top)-(pMtr->radius)-GetTextHeight(pScheme->pFont))>>1);
	//pMtr->yCenter = bottom - (((bottom - top)-(pMtr->radius+(GOL_EMBOSS_SIZE<<1))-tempHeight)>>1);

#elif defined (MTR_QUARTER_TYPE)
	// choose the radius
	if ((right-left) > (bottom-top))
		pMtr->radius = 	(bottom-top) - 
						(GOL_EMBOSS_SIZE+(tempHeight<<1)+
							(((bottom-top)+GOL_EMBOSS_SIZE+(tempHeight<<1))>>2));
	else	
		pMtr->radius =  (right-left) - (GOL_EMBOSS_SIZE+tempWidth+(((right-left)+GOL_EMBOSS_SIZE+tempWidth)>>2));

	// center the meter on the given dimensions
	pMtr->xCenter = left + (((right-left)-(pMtr->radius+tempWidth+GOL_EMBOSS_SIZE))>>1);
	pMtr->yCenter = bottom - (GOL_EMBOSS_SIZE + tempHeight);
...
typedef enum {
	...
	static XCHAR strVal[SCALECHARCOUNT] = "";
	float radian;
	DWORD_VAL dTemp, dRes;
    ...
    switch(state) {
        ...
        case TEXT_DRAW:	
			...
			// set the start location of the meter title
				...
				MoveTo( pMtr->xCenter-(temp>>1), pMtr->yCenter+(GetTextHeight(pMtr->pGolScheme->pFont)*3));
				...
				MoveTo(((pMtr->right+pMtr->left)>>1)-(temp>>1), pMtr->yCenter+3);
...
mtr_needle_draw_st:
		...
		case VALUE_ERASE:
			...
			temp = GetTextWidth("888", pMtr->pGolScheme->pFont);
			SetColor(pMtr->pGolScheme->Color0);
		
			#if defined (MTR_WHOLE_TYPE) 
		
				Bar(    pMtr->xCenter-(temp>>1),
		 				pMtr->yCenter+(GetTextHeight(pMtr->pGolScheme->pFont)<<1),
				    	pMtr->xCenter+(temp>>1),
		 				pMtr->yCenter+(GetTextHeight(pMtr->pGolScheme->pFont)*3));
