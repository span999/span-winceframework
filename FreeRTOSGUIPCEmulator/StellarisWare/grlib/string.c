...
//
//*****************************************************************************
unsigned long NumLeadingZeros(unsigned long val)
{
  int count;
...
GrStringGet(int iIndex, char *pcData, unsigned long ulSize)
{
    ...
    while(iPos >= 0)
    {
        ...
        else if(ulOffset & SC_FLAG_COMPRESSED)
        {
            ...
            // compressed data.
            //
            pcString = g_pucStringData + (ulOffset & SC_OFFSET_M);
            ...
            //
            pcBufferOut = (unsigned char *)pcData + iIdx;
            ...
            for(; ulLen; ulLen--)
            {
                ...
                //
                *pcBufferOut = (*pcString >> iBit) & 0x3f;

                if(iBit >= 2)
                {
                    *pcBufferOut |= (*++pcString << (8-iBit)) & 0x3f;
                }
        ...
        else if(SC_GET_INDEX(ulSubCode[0]) && !SC_GET_LEN(ulSubCode[0]))
        {
            ...
            //
            pcData[iIdx + SC_GET_INDEX(ulSubCode[0])] = 0;
        }
        else
        {
            //
    ...
    if(pcBufferOut)
    {
        ulLen = ((unsigned long)pcBufferOut - (unsigned long)pcData);
    }
    else
    {
        ulLen = 0;
