...
ImageButtonPaint(tWidget *pWidget)
{
    ...
    //
    // If there is a keycap image, center this on the top of the button,
    // applying any offset defined if the button is currently pressed.
    //
    if(pPush->pucKeycapImage)
    {
        //
...
ImageButtonClick(tWidget *pWidget, unsigned long ulMsg, long lX, long lY)
{
    ...
    if(ulMsg == WIDGET_MSG_PTR_UP)
    {
        ...
        //
        // If an additional image has been defined to be used when the
        // button is pressed, redraw the button to replace this with the
        // release state image.
        //
        if(pPush->pucPressImage)
        {
    ...
    if(GrRectContainsPoint(&pWidget->sPosition, lX, lY))
    {
        ...
        if(ulMsg == WIDGET_MSG_PTR_DOWN)
        {
            ...
            //
            // If an additional image has been defined to be used when the
            // button is pressed, redraw the button accordingly.
            //
            if(pPush->pucPressImage)
            {
                ImageButtonPaint(pWidget);
