...
GotoAppVector:
    ...
    btfsc   PORTC, RC7          ; BREAK found, wait for RXD to go IDLE
    bra     $-2
#else
    btfsc   PORTC, RC7          
GotoAppVector:
    ...
    btfss   PORTC, RC7          ; BREAK found, wait for RXD to go IDLE
    bra     $-2
#endif
...
BootloaderStart:
    ...
    btfsc   PORTC, RC7          
    bra     WaitForRxIdle
    bra     CheckAppVector      ; no BREAK state, attempt to start application
#else
    btfss   PORTC, RC7          
    bra     WaitForRxIdle
    bra     CheckAppVector      ; no BREAK state, attempt to start application
...
RetryAutoBaud:
    ...
    movff   TMR0L, SPBRG        ; warning: must read TMR0L before TMR0H holds real data
    movff   TMR0H, SPBRGH
#else 
    movff   TMR0L, SPBRG        ; warning: must read TMR0L before TMR0H holds real data
...
#else
    movlw   FIXED_SPBRG         ; setup fixed baud rate
    movwf   SPBRG
    bsf     RCSTA, CREN         ; start receiving
DoAutoBaud:
WaitForHostCommand:
...
WaitForRiseLoop:
    ...
    btfss   PORTC, 7        ; Wait for a falling edge
    bra     WaitForRiseLoop

WtSR:
    btfsc   PORTC, 7        ; Wait for starting edge
    bra     WtSR
    return
#else
WaitForRise:
...
WtSR:
    btfss   PORTC, 7        ; Wait for rising edge
    bra     WtSR
    return
#endif
; *****************************************************************************
