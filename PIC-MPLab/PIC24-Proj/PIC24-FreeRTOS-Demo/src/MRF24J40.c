...
*  2.1a  09/04/09     dw        Modified for RTOS demo
*                               a semaphore has been added to the
*                               RF interrupt handler to indicate to
*                               the MiWi task when a packet _may_ need
*                               processing                             
********************************************************************/
...
// semaphore used to wake the MiWitask up because some activity
// has occured in the stack, this allows that task to spend longer
// blocked whilst still responding quickly to arriving messages
extern xSemaphoreHandle hMiWiSemaphore;
    ...
    {   
        ...
        {
            if(failureCounter >= (unsigned) FAILURE_COUNTER)
            {
                failureCounter = 0;
        ...
        if(MRF24J40Status.bits.RX_BUFFERED == 1u)
        {
            BYTE addrMode;
            #ifndef TARGET_SMALL
                BOOL bIntraPAN = TRUE;
            
                if( (RxBuffer[1] & 0x40) == 0u )
                {
                    bIntraPAN = FALSE;   
                }
            #endif
                        ...
                        INPUT BYTE MACPayloadLen)
    {
        ...
        if( transParam.flags.bits.packetType == (unsigned) PACKET_TYPE_COMMAND )
        {
            i = 0x03;
        }
        else if( transParam.flags.bits.packetType == (unsigned) PACKET_TYPE_DATA)
        {
            i = 0x01;
        }
        ...
        if( transParam.flags.bits.packetType == (unsigned) PACKET_TYPE_RESERVE)
        {
            i = 0x00;
            headerLength = 7;
        ...
        PHYSetShortRAMAddr(WRITE_TXNMTRIG, i); 
            ...
            {   
                ...
                if( MRF24J40Status.bits.TX_BUSY == 0u )
                {
    ...
    #endif
    {
        // a flag indicating if a higher priority task may have been woken
        // because of actions taken during this ISR
        ...
        {  
            ...
            //flag quickly and then handle the interrupt that we have already received
            {
                ...
                {  
                    ...
                    if(MRF24J40Status.bits.RX_BUFFERED == 0u)
                    {
                        #ifdef ENABLE_SECURITY
                        ...
                        if(RxBuffer[0]< (unsigned) RX_PACKET_SIZE)
                        {   
                            ...
                            ///////////////////////////////////////////////////////////
                            // data has been received so unblock any tasks
                            // waiting on the miwi semaphore (i.e. the MiWi taskitself)
		                    xSemaphoreGiveFromISR(hMiWiSemaphore, &taskWoken);	
                        }
                        else
                        {
