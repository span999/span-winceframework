...
#include <windows.h>
#include "spCElogCRC.h"                /* CRC typedefs and definitions         */
...
*/
const UINT16 crc16_table[ CRC_TAB_SIZE ] = {
...
)
{
  ...
  */
  for (crc_16 = CRC_16_SEED ; len >= 8; len -= 8, buf_ptr++) {
    crc_16 = (UINT16)(crc16_table[ (crc_16 >> (16 - 8)) ^ *buf_ptr ] ^ (crc_16 << 8));
  }
...
UINT16 FindCRC( BYTE *pData, DWORD dwLen )
{
	UINT16 uiRet = 0;
	UINT16 uiCRC = CRC_16_L_SEED;
	DWORD dwCount = dwLen; 
	BYTE bThisByte = 0;
	BYTE *pThisByte = pData;
	...
	if( pData && pData )
	{
		...
		while( dwCount > 0 )
		{
			...
			if( (bThisByte == DIAG_ASYNC_ESC_CHAR) )
			{
				/* If the escape character is not the last byte */
				if( dwCount != 1 ) 
				{
					///uiCRC = CRC_16_L_STEP( uiCRC, bThisByte );
				...
				///skip and get next one~
				if( dwCount > 0 )
				{
					dwCount--;
					pThisByte++;
					bThisByte = *pThisByte;
					bThisByte = bThisByte ^ DIAG_ASYNC_ESC_MASK;
					uiCRC = CRC_16_L_STEP( uiCRC, bThisByte );
				}
			}
			else
        ...
        if( 1 ) ///while ( 0 )
        {
			...
			bThisByte = uiCRC & 0xFF;
  
			/* Copied from above for runtime efficiency */
			if ( (bThisByte == DIAG_ASYNC_CONTROL_CHAR) || (bThisByte == DIAG_ASYNC_ESC_CHAR) )
			{
				///if( dest != dest_last ) 
				{
					///*dest++ = DIAG_ASYNC_ESC_CHAR;
