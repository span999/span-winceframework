...
*  1.0a	 09/11/2008   D.Wenn	Modified for RTOS demo
*  2.0   4/15/2009    yfy       MiMAC and MiApp revision
*  2.1   6/20/2009    yfy       Add LCD support
*  2.1a  09/04/09     DW        Updated for RTOS demo
********************************************************************/
...
// Console output is via the UART task
#include ".\Common\Console.h"
// Rather than use the standard provided time function we will
// provide our own Tick handling functions but use the common header
 ...
 **********************************************************************/
#if defined(__18CXX)
    #pragma udata BIGvariables1 = 0x600
#endif
...
BOOL MiApp_ProtocolInit(void)
{
    ...
    // the intialization of the symbol timer and tick timer is performed in main.c
    //InitSymbolTimer();
    ...
    { 
        ...
        for(i = 0; i < INDIRECT_MESSAGE_SIZE; i++)
        {
            if( indirectMessages[i].flags.bits.isValid == 0 )
            {
                ...
                indirectMessages[i].TickStart = MiWiTickGet();
                return TRUE;
            }
        }
        return FALSE;
