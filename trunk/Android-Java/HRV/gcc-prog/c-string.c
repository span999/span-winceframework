...
#ifdef TESTC
static int GetRandomTagStr( unsigned char **pStr )
#else
...
#ifdef TESTC
static int GetRandomRAWDATAStr( unsigned char **pStr )
#else
...
#ifdef TESTC
int GetRandomStr( unsigned char **pStr )
#else
...
#endif
{
	...
	if( 1 )
	{
		...
		else
		{
			...
			///pTemp = pTemp + 6;	///shift
#ifdef TESTC
			iRet = GetRandomRAWDATAStr( &pcTemp );
#else
